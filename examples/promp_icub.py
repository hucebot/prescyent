...
from prescyent.predictor.promp.promp import PrompPredictor
...
if __name__ == "__main__":
    ...
    predictor = PrompPredictor(dataset_config)
    print("OK")
    ...
    for p in range(dataset.trajectories.test[0].tensor.size(1)):
        for n in range(dataset.trajectories.test[0].tensor.size(2)):
            ...
            for i,traj in enumerate(dataset.trajectories.test):
                ax.plot(np.arange(0, traj.tensor.size(0)), traj.tensor[:,p,n], label=str(i))
            fig.legend()
