...
def get_predictor_infos(config: Dict[str, Any]) -> Type[BasePredictor]:
    """retrieve predictor class from a config
    ...
    Returns:
        Type[BasePredictor]: retrieved child class of a BasePredictor
    """

    predictor_class_name = config.get("predictor_class", None)
    if predictor_class_name is None:
        predictor_class_name = config.get("model_config", None).get("predictor_class")
    if predictor_class_name is None:
        logger.getChild(PREDICTOR).error("Could not find a predictor class in config")
        raise AttributeError(predictor_class_name)
    module_name, class_name = predictor_class_name.rsplit(".", 1)
