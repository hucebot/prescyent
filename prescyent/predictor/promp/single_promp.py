...
class SinglePromp:
    ...
    def __init__(self, mean_w: torch.Tensor=None, std_w: torch.Tensor=None, data=None):
        # parameters
        self.num_bf = 10# lower = smoother
        self.std_bf =  1.0 / (self.num_bf**2)
        self.ridge_factor = 1e-10 # default value
        ...
        if mean_w != None:
            ...
            self.phase = torch.linspace(0, 1, math.floor(self.s / self.alpha)) # should be phase_vec
            self.psi = self.gen_basis_functions(self.phase) # should psi_mtx

    def clone(self):
        return SinglePromp(self.mean_w.clone(), self.std_w.clone(), self.data.clone())
    ...
    def gen_basis_functions(self, phase : torch.Tensor):
        ...
        centers = torch.linspace(0.0 - 2 * self.std_bf , 1.0 + 2 * self.std_bf, self.num_bf)
    ...
    def train(self, trajs : list[torch.Tensor]):
        # data pre-processing
        mean_length = math.floor(functools.reduce(lambda x,y : x + y.size(0), trajs, 0) / float(len(trajs)))
        ...
        psi_t_inv = (self.psi @ self.psi.T + self.ridge_factor * id).inverse() @ self.psi
        ...
        for i in range(0, self.data.size(0)):
            w_mtx[:, i] = psi_t_inv @ self.data[i, :] # is this type converted to a float?
    ...
    def condition(self, traj: torch.Tensor, std: torch.Tensor=None):
        ...
        #self.mean_w = mean_w
        #self.std_w = std_w
        return SinglePromp(mean_w, std_w, self.data.clone())
