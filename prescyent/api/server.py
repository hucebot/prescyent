...
def return_500_http_except(func):
    @wraps(func)
    async def wrapper(*args, **kwargs):
        try:
            ...
            raise HTTPException(500,
                                f"{generic_exception.__class__.__name__}: {generic_exception}"
...
class PredictorApi():
    def __init__(self, predictor_path:BasePredictor) -> None:
        ...
        self.router.add_api_route("/update_predictor", self.update_predictor, methods=["GET"])
        self._app.include_router(self.router)

    @return_500_http_except
    async def predict(self, tensor: list, future:int):
        tensor = torch.tensor(tensor)
        return self.predictor.predict(tensor, future_size=future).tolist()

    @return_500_http_except
    async def update_predictor(self, predictor_path):
