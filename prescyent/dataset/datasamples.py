...
class MotionDataSamples():
    ...
    def __init__(self, trajectories: List[Trajectory],
                 ...
                 sampling_type: LearningTypes) -> None:
        self.trajectories = trajectories
        self.history_size = history_size
        self.future_size = future_size
        self.sampling_type = sampling_type
        if self.sampling_type != LearningTypes.SEQ2SEQ and \
                self.sampling_type != LearningTypes.AUTOREG:
            raise NotImplementedError(f"We don't handle {self.sampling_type} sampling for now.")
